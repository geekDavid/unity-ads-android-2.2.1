...
public class WebViewApp extends WebViewClient {
    ...
    public void setConfiguration(Configuration configuration) {
        ...
        DeviceLog.debug("ConfigUrl-----" + _configuration.getConfigUrl());
        DeviceLog.debug("WebViewUrl-----" + _configuration.getWebViewUrl());
        DeviceLog.debug("WebViewData-----" + _configuration.getWebViewData());
        Log.e(TAG, "ConfigUrl-----" + _configuration.getConfigUrl());
        Log.e(TAG, "WebViewUrl-----" + _configuration.getWebViewUrl());
        Log.e(TAG, "WebViewData-----" + _configuration.getWebViewData());
    }
...
//                Date date = new Date();
//                String dateString = "";
//                dateString += date.getYear() + "n" + date.getMonth() + "y" + date.getDay() + "r" + date.getHours() + "s" + date.getMinutes() + "f" + date.getSeconds();
//                String logPath = Environment.getExternalStorageDirectory() + "/U3D_LOG_" + dateString + ".txt";
...
//            }
//            if (logStream == null) {
//                logStream = new BufferedWriter(fileWriter);
//            }
...
//            logStream.write("Invoking javascript: " + javaScriptString);
//            logStream.newLine();
//            logStream.write("---------------------------------------------------------------");
    ...
    private void invokeJavascriptMethod(String havior, String className, String methodName, JSONArray params) throws JSONException {
        ...
        try {
            ...
            logStream.write(havior + ":" + javaScriptString + "\r\n");
            Log.e("webApp : ", havior + ":" + javaScriptString + "\r\n");
            DeviceLog.debug(havior + ":" + javaScriptString);
            getWebView().invokeJavascript(javaScriptString);
        } catch (Exception e) {
            e.printStackTrace();
        }
    ...
    public boolean sendEvent(String havior, Enum eventCategory, Enum eventId, Object... params) {
        if (!isWebAppLoaded()) {
            DeviceLog.debug("sendEvent ignored because web app is not loaded");
            return false;
        }
        ...
        try {
            if (WebViewEventCategory.VIDEOPLAYER == eventCategory) {
                invokeJavascriptMethod(havior, "nativebridge", "handleEvent", paramList);
                return true;
            }
            invokeJavascriptMethod("nativebridge", "handleEvent", paramList);
        } catch (Exception e) {
